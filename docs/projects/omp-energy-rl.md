---
title: OpenMP energy efficiency
---
...
hardware performance counters — architecture-specific, and counting events
...
only, with no say over core count — and that axis matters: the compute-bound
benchmark peaks at 19 cores, the memory-bound one at **a single core**, its
memory subsystem already saturated so the rest add power and no throughput.
...
applications need no source changes. Energy comes from RAPL registers, package
and DRAM over both sockets — and from board current sensors on the Odroid XU3
used for ReCoSoC 2019, which is the portability argument: only that half of the
metric changed.
...
`[CpS, frequency, core count]` through a bottleneck forced to be *discrete* — a
...
1000 samples, 210 000 rows — in about a minute on CPU, and its 2-bit code
...
Two numbers come back out of the data too: the DGEMM sweep reduces to 19 cores
at 2.1 GHz, mean CpJ 166.2, against the thesis's 18 cores at 2.1 GHz, mean
CpJ 166; and recomputed gains over the four Linux governors on DGEMM are
11.7 %, 18.4 %, 12.3 % and 12.1 % against the thesis's 10 %, 17 %, 11 % and
10 %. Both are consistency checks on the bundled data, not independent
replications — the underlying measurements are the original ones.
...
reconstructions, and one benchmark's argument *meanings* are inferred from an
original invocation. The step fusing the chunk stream into CpS/CpJ is rebuilt
from the thesis definitions. The scaler fitted alongside the published phase
encoder is not available, so that checkpoint is not usable on its own. Intel PCM
support is absent, so the radar charts of thesis Fig. 4.13 cannot be
regenerated.
